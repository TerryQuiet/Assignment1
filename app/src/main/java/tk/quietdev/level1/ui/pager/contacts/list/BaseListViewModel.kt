...
abstract class BaseListViewModel (
    ...
    protected fun searchQueryMap(it: Resource<List<UserModel>>): Resource<List<UserModel>> =
        when (it) {
            ...
            is Resource.Success -> Resource.Success((it.data?.filter { it.email.startsWith(search, true) }
                ?: listOf()))
            is Resource.Loading -> Resource.Loading((it.data?.filter { it.email.startsWith(search, true) }
                ?: listOf()))
        }
